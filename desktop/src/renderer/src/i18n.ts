...
export const messages: Record<string, Entry> = {
  ...
  'mcp.addHint': {
    ko: 'stdio는 로컬에서 명령으로 실행되는 MCP 서버, http는 이미 떠 있는 MCP 엔드포인트입니다. 추가 후 “테스트”로 연결과 도구 목록을 확인하세요. 에이전트는 {list}로 도구를 찾고 {call}로 호출합니다.',
    en: 'stdio is an MCP server launched locally by a command; http is an already-running MCP endpoint. After adding, use “Test” to verify the connection and tool list. The agent discovers tools with {list} and invokes them with {call}.',
  },
