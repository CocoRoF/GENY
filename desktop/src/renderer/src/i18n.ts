...
export const messages: Record<string, Entry> = {
  ...
  'drive.disconnectWarn': { ko: '{agent} 을(를) GenyDrive에서 분리하면 공유 중인 폴더({folders})에 더 이상 접근할 수 없게 되며, 이 에이전트의 저장소에서 해당 폴더 사본이 제거됩니다. 내 컴퓨터의 원본 폴더는 그대로입니다. 계속할까요?', en: 'Disconnecting {agent} from GenyDrive ends its access to the shared folders ({folders}) and removes their copies from this agent’s storage. Your own folders on this computer are untouched. Continue?' },
  ...
  'sync.unlinkWarn': { ko: '연결을 해제하면 이 폴더를 공유 중인 에이전트 {count}개가 더 이상 접근할 수 없게 되며, 각 에이전트 저장소에서 사본이 제거됩니다. 내 컴퓨터의 원본 폴더는 그대로입니다. 계속할까요?', en: 'Unlinking ends access for the {count} agent(s) sharing this folder and removes their copies. Your own folder on this computer is untouched. Continue?' },
