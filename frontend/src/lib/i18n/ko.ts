...
const ko: Translations = {
  ...
  googleSettings: {
    ...
    guideStep4: '"사용자 인증 정보 → 사용자 인증 정보 만들기 → OAuth 클라이언트 ID"에서 애플리케이션 유형을 "TV 및 입력 제한 기기"로 선택해 생성합니다. (이 유형이라야 기기 인증이 됩니다)',
    guideStep5: '생성된 클라이언트 ID와 클라이언트 시크릿을 아래 칸에 붙여넣고 "저장"을 누릅니다.',
    guideStep6: '"연결"을 누른 뒤 표시되는 주소를 열어 코드를 입력하고 권한을 승인하면 끝입니다.',
    guideOpenConsole: 'Google Cloud Console 열기',
    clientSection: 'OAuth 클라이언트',
    clientSet: '설정됨',
    clientUnset: '미설정',
    clientHelper: '위 안내에 따라 만든 "TV 및 입력 제한 기기" 유형 OAuth 클라이언트의 ID와 시크릿을 붙여넣으세요.',
    clientId: '클라이언트 ID',
