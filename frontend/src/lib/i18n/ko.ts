...
const ko: Translations = {
  ...
  settings: {
    ...
    llmBackends: {
      navLabel: 'LLM 백엔드',
      title: 'LLM 백엔드',
      description: '6개 프로바이더가 executor의 ClientRegistry에 매핑됩니다. 각 카드가 해당 프로바이더의 유일한 편집창이에요 — API 키를 붙여넣거나(Anthropic / OpenAI / Google / vLLM) 모달 내 로그인 흐름을 실행하면(Claude Code, Copilot) 됩니다. 카드가 초록색이 되면 해당 프로바이더를 Stage 6에서 선택한 Environment가 실제 VTuber / Worker 세션을 그 백엔드로 라우팅합니다.',
      refreshAll: '전체 새로고침',
      reCheck: '다시 확인',
      docs: '문서',
      ...
      detail: {
        ...
        'copilot.login_required': '{path} · {version} · `gh auth login` 필요.',
        'copilot.extension_missing': '{path} · {version} · `github/gh-copilot` 확장 설치 필요.',
      } as Record<string, string>,
      installHelp: {
      ...
      copilotModal: {
        ...
        ghAuthStatusPrefix: ': ',
        extensionLabel: '확장',
        extensionMissingIntro: 'GitHub에는 로그인되어 있지만 Copilot CLI 확장이 설치되지 않았습니다. 호스트에 SSH 접속해서 실행하세요:',
        extensionMissingOutro: '그 후 위의 새로고침을 누르세요.',
        signOutConfirm: '서버에서 gh 로그아웃할까요? Copilot CLI를 다시 사용하려면 재로그인해야 합니다.',
      },
    },
  },
