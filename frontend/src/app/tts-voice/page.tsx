...
function ProfileDetail({
  profile, onActivate, onRefresh, showMsg, t,
}: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8 space-y-8">
      ...
      <div className="flex items-center gap-3">
        {isTemplate && (
          <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[rgba(59,130,246,0.1)] text-[var(--primary-color)] text-[0.75rem] font-medium border border-[rgba(59,130,246,0.2)]">
            {t('ttsVoice.templateBadge')}
          </span>
        )}
        {!profile.active && (
          <button
...
function EmotionRefCard({
  profileName, emotion, hasRef, emotionRef, uploading, isTemplate, onUpload, onDelete, onUpdatePrompt, t,
}: {
  profileName: string;
  emotion: Emotion;
  hasRef: boolean;
            ...
            <div className="flex-1">
              ...
              {isTemplate ? (
                <p className="px-2.5 py-1.5 text-[0.75rem] text-[var(--text-primary)]">
                  {localPromptText || <span className="text-[var(--text-muted)] italic">{t('ttsVoice.noPromptText')}</span>}
                </p>
              ) : (
                <input
