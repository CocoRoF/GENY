"""
TTS configuration category.

This category contains configs for TTS (Text-to-Speech) engine settings:
- General: Provider selection, emotion mapping, audio format, cache
- Edge TTS: Free Microsoft TTS voice settings
- OpenAI TTS: OpenAI API settings
- ElevenLabs: Voice cloning + emotion settings
- GPT-SoVITS: Open-source voice cloning with emotion references
- Fish Speech: Open-source fast TTS
"""
