...
> 작성일: 2026-04-04
> 목적: VTuber/CLI 모드별 Tool 고도화를 위한 현행 시스템 분석
...
- VTuber든 CLI든 **Claude CLI subprocess 자체가 MCP를 통해 tool에 접근**
...
## 6. VTuber와 CLI의 Tool 사용 방식

### 6.1 핵심: 둘 다 Tool을 직접 사용한다

VTuber와 CLI 모두 Claude CLI subprocess를 통해 tool을 **직접 자율적으로** 사용함.
...
| **CLI 위임** | vtuber_delegate 노드가 linked CLI 세션에 DM으로 task 전달 → CLI가 autonomous workflow로 실행 | 코드 작성, 파일 조작, 복잡한 멀티스텝 작업 |
...
```
┌─────────────────────────────────────────────────────────┐
│  CLI Workflow: template-optimized-autonomous             │
│                                                         │
...
### 6.4 VTuber → CLI 위임 상세 흐름

```
User: "새 프로젝트 구조 만들어줘"
         │
         ▼
VTuber classify: "delegate_to_cli" (코드/파일 작업 필요)
         │
         ▼
VTuber delegate_node:
  1. linked CLI session ID 조회
  2. LLM으로 task 재구성 ("새 프로젝트 구조를 생성하세요")
  3. 최근 5개 대화 맥락 추출 (각 300자 제한)
  4. DM 전송 → CLI 세션 inbox
...
| `delegate` | vtuber_delegate | CLI 세션에 작업 위임 |
...
| `delegate_to_cli` | 코드 작성, 파일 작업, 쉘 명령, 복잡한 리서치 | → delegate |
| `thinking` | `[THINKING_TRIGGER]` 또는 `[CLI_RESULT]` prefix | → think |
...
**VTuber와 CLI 구분 없이** 동일한 함수 사용. preset 설정에 의해서만 차이 발생.
...
| CLI tool 사용 | **직접 사용** (동일 메커니즘) |
| Built-in 필터링 | **불가능** — 프리셋 시스템이 built-in을 필터링 안 함 |
| VTuber/CLI tool 차이 | **없음** — 둘 다 동일한 25개 tool에 접근 |

### 10.2 VTuber와 CLI에서 각 Tool의 적합성 분석

| Tool | VTuber 적합성 | CLI 적합성 | 비고 |
|------|-------------|----------|------|
| `web_search` | ✅ 높음 — 일상 대화 중 검색 | ✅ 높음 — 리서치 | 양쪽 모두 유용 |
| `news_search` | ✅ 높음 — "요즘 뉴스 뭐 있어?" | ✅ 보통 | VTuber에 더 자연스러움 |
| `web_fetch` | ⚠️ 보통 — 가끔 유용 | ✅ 높음 — 데이터 수집 | |
| `web_fetch_multiple` | ❌ 낮음 — VTuber가 병렬 페치? | ✅ 높음 — 대량 수집 | CLI 전용이 적절 |
| `browser_navigate` | ❌ 낮음 — 일상 대화에 불필요 | ✅ 높음 — 웹 자동화 | CLI 전용이 적절 |
| `browser_click` | ❌ 낮음 | ✅ 높음 — UI 자동화 | CLI 전용이 적절 |
| `browser_fill_form` | ❌ 낮음 | ✅ 높음 — 자동화 | CLI 전용이 적절 |
...
| `geny_session_create` | ❌ 낮음 — VTuber가 세션 생성? | ✅ 높음 — 자동 스케일링 | CLI 전용이 적절 |
| `geny_room_create` | ❌ 낮음 | ✅ 높음 — 협업 구조 | CLI 전용이 적절 |
...
1. **새 tool을 만들면 VTuber/CLI 구분 없이 모두에게 노출**
...
| `backend/service/workflow/nodes/vtuber/vtuber_delegate_node.py` | VTuber → CLI 위임 |
...
| `backend/workflows/template-optimized-autonomous.json` | CLI workflow 정의 |
...
[세션 생성 — VTuber든 CLI든 현재 동일 흐름]
    ...
    │   └─ CLI   → template-optimized-autonomous (classify → easy/batch_execute)
    │
    └─ .mcp.json 저장 → Claude CLI가 사용

[Tool 실행 — VTuber/CLI 모든 노드에서 가능]
    │
